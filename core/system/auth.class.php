...
class Auth
{
    ...
    {
        $this->dbh = new \PDO("mysql:host=" . Config::DB['host'] . ";dbname=" . Config::DB['name'], Config::DB['user'], Config::DB['password']);
    ...
    public function activate($key)
    {
        ...
        if($this->getBaseUser($getRequest['uid'])['status'] == 1) {
    ...
    private function addRequest($uid, $email, $type, &$sendmail)
    {
        ...
        if($type == "activation" && $this->getBaseUser($uid)['status'] == 1) {
...
//				return $return;
    ...
    public function resendActivation($email, $sendmail = NULL)
    {
        ...
        if ($this->getBaseUser($row['id'])['status'] == 1) {
            $return['message'] = $this->lang["already_activated"];
            return $return;
        }
    ...
    public function isBlocked()
    {
        ...
        if($attempts < intval(5))
        {
            return "allow";
        }
        if($attempts < intval(30))
        {
            return "verify";
        }
        return "block";
